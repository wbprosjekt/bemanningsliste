...
CREATE UNIQUE INDEX idx_ref_chargers_unique ON ref_chargers(org_id, easee_charger_id) WHERE easee_charger_id IS NOT NULL;
CREATE INDEX idx_ref_chargers_org ON ref_chargers(org_id);

-- 2. RFID Keys table
CREATE TABLE ref_rfid_keys (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  org_id UUID NOT NULL REFERENCES public.org(id) ON DELETE CASCADE,
...
CREATE UNIQUE INDEX idx_ref_rfid_keys_unique ON ref_rfid_keys(org_id, easee_key_id);
CREATE INDEX idx_ref_rfid_keys_org ON ref_rfid_keys(org_id);

-- 3. Employee-RFID-Charger mapping table
CREATE TABLE ref_employee_keys (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  org_id UUID NOT NULL REFERENCES public.org(id) ON DELETE CASCADE,
...
CREATE UNIQUE INDEX idx_ref_employee_keys_unique ON ref_employee_keys(org_id, profile_id, rfid_key_id);
CREATE INDEX idx_ref_employee_keys_profile ON ref_employee_keys(profile_id);
CREATE INDEX idx_ref_employee_keys_rfid ON ref_employee_keys(rfid_key_id);
CREATE INDEX idx_ref_employee_keys_charger ON ref_employee_keys(charger_id);

-- 4. Nett profiles (TOU nettleie)
CREATE TABLE ref_nett_profiles (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  org_id UUID NOT NULL REFERENCES public.org(id) ON DELETE CASCADE,
  name TEXT NOT NULL,
  timezone TEXT NOT NULL DEFAULT 'Europe/Oslo',
...
CREATE INDEX idx_ref_nett_profiles_org ON ref_nett_profiles(org_id);

-- 5. Effect tiers (trinn for effektledd)
CREATE TABLE ref_effect_tiers (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  org_id UUID NOT NULL REFERENCES public.org(id) ON DELETE CASCADE,
  name TEXT NOT NULL,
...
CREATE INDEX idx_ref_effect_tiers_org ON ref_effect_tiers(org_id);

-- 6. Employee settings (prisstrategi per ansatt)
CREATE TABLE ref_employee_settings (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  org_id UUID NOT NULL REFERENCES public.org(id) ON DELETE CASCADE,
  profile_id UUID NOT NULL REFERENCES profiles(id) ON DELETE CASCADE,
...
CREATE UNIQUE INDEX idx_ref_employee_settings_profile ON ref_employee_settings(profile_id, effective_from);
CREATE INDEX idx_ref_employee_settings_org ON ref_employee_settings(org_id);

-- 7. Energy prices (spot cache)
CREATE TABLE ref_energy_prices (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  org_id UUID NOT NULL REFERENCES public.org(id) ON DELETE CASCADE,
...
CREATE UNIQUE INDEX idx_ref_energy_prices_unique ON ref_energy_prices(org_id, provider, area, ts_hour);
CREATE INDEX idx_ref_energy_prices_lookup ON ref_energy_prices(org_id, area, ts_hour);

-- 8. Nett windows (TOU timeintervaller)
CREATE TABLE ref_nett_windows (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
...
CREATE INDEX idx_ref_nett_windows_profile ON ref_nett_windows(profile_id);

-- 9. Raw CSV sessions (for audit trail)
CREATE TABLE ref_sessions_raw (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  org_id UUID NOT NULL REFERENCES public.org(id) ON DELETE CASCADE,
...
CREATE INDEX idx_ref_sessions_raw_lookup ON ref_sessions_raw(org_id, employee_id, period_month);
CREATE INDEX idx_ref_sessions_raw_hash ON ref_sessions_raw(csv_hash);

-- 10. Hourly split sessions (for pricing)
CREATE TABLE ref_sessions_hourly (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  org_id UUID NOT NULL REFERENCES public.org(id) ON DELETE CASCADE,
  employee_id UUID NOT NULL REFERENCES profiles(id) ON DELETE CASCADE,
  period_month DATE NOT NULL,
...
CREATE INDEX idx_ref_sessions_hourly_lookup ON ref_sessions_hourly(org_id, employee_id, period_month);
CREATE INDEX idx_ref_sessions_hourly_time ON ref_sessions_hourly(local_ts_hour);

-- 11. Reimbursements (generated reports)
CREATE TABLE ref_reimbursements (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  org_id UUID NOT NULL REFERENCES public.org(id) ON DELETE CASCADE,
  employee_id UUID NOT NULL REFERENCES profiles(id) ON DELETE CASCADE,
  period_month DATE NOT NULL,
...
CREATE UNIQUE INDEX idx_ref_reimbursements_unique ON ref_reimbursements(org_id, employee_id, period_month);
CREATE INDEX idx_ref_reimbursements_employee ON ref_reimbursements(employee_id);
...
CREATE UNIQUE INDEX idx_profile_modules_unique ON profile_modules(profile_id, module_name);
CREATE INDEX idx_profile_modules_lookup ON profile_modules(profile_id, module_name, enabled);
...
CREATE POLICY ref_chargers_select ON ref_chargers FOR SELECT
  USING (auth.jwt() ->> 'role' IN ('admin', 'økonomi') OR EXISTS (
    SELECT 1 FROM profiles WHERE id = auth.uid() AND org_id = ref_chargers.org_id
  ));

CREATE POLICY ref_chargers_insert ON ref_chargers FOR INSERT
  WITH CHECK (auth.jwt() ->> 'role' IN ('admin', 'økonomi'));

CREATE POLICY ref_chargers_update ON ref_chargers FOR UPDATE
  USING (auth.jwt() ->> 'role' IN ('admin', 'økonomi'));

CREATE POLICY ref_chargers_delete ON ref_chargers FOR DELETE
  USING (auth.jwt() ->> 'role' IN ('admin', 'økonomi'));

-- RFID Keys: admin/økonomi ser allt i org, ansatt ser ikke
CREATE POLICY ref_rfid_keys_select ON ref_rfid_keys FOR SELECT
  USING (auth.jwt() ->> 'role' IN ('admin', 'økonomi'));

CREATE POLICY ref_rfid_keys_insert ON ref_rfid_keys FOR INSERT
  WITH CHECK (auth.jwt() ->> 'role' IN ('admin', 'økonomi'));
CREATE POLICY ref_rfid_keys_update ON ref_rfid_keys FOR UPDATE
  USING (auth.jwt() ->> 'role' IN ('admin', 'økonomi'));
CREATE POLICY ref_rfid_keys_delete ON ref_rfid_keys FOR DELETE
  USING (auth.jwt() ->> 'role' IN ('admin', 'økonomi'));

-- Employee Keys: admin/økonomi ser allt i org, ansatt ser ikke
CREATE POLICY ref_employee_keys_select ON ref_employee_keys FOR SELECT
  USING (auth.jwt() ->> 'role' IN ('admin', 'økonomi'));

CREATE POLICY ref_employee_keys_insert ON ref_employee_keys FOR INSERT
  WITH CHECK (auth.jwt() ->> 'role' IN ('admin', 'økonomi'));
CREATE POLICY ref_employee_keys_update ON ref_employee_keys FOR UPDATE
  USING (auth.jwt() ->> 'role' IN ('admin', 'økonomi'));
CREATE POLICY ref_employee_keys_delete ON ref_employee_keys FOR DELETE
  USING (auth.jwt() ->> 'role' IN ('admin', 'økonomi'));

-- Employee Settings: ansatt ser kun egne, admin/økonomi ser allt i org
CREATE POLICY ref_employee_settings_select ON ref_employee_settings FOR SELECT
  USING (profile_id = auth.uid() OR auth.jwt() ->> 'role' IN ('admin', 'økonomi'));

CREATE POLICY ref_employee_settings_insert ON ref_employee_settings FOR INSERT
  WITH CHECK (auth.jwt() ->> 'role' IN ('admin', 'økonomi'));
CREATE POLICY ref_employee_settings_update ON ref_employee_settings FOR UPDATE
  USING (auth.jwt() ->> 'role' IN ('admin', 'økonomi'));
CREATE POLICY ref_employee_settings_delete ON ref_employee_settings FOR DELETE
  USING (auth.jwt() ->> 'role' IN ('admin', 'økonomi'));
...
CREATE POLICY ref_sessions_raw_select ON ref_sessions_raw FOR SELECT
  USING (employee_id = auth.uid() OR auth.jwt() ->> 'role' IN ('admin', 'økonomi'));

CREATE POLICY ref_sessions_raw_insert ON ref_sessions_raw FOR INSERT
  WITH CHECK (auth.jwt() ->> 'role' IN ('admin', 'økonomi'));
CREATE POLICY ref_sessions_raw_update ON ref_sessions_raw FOR UPDATE
  USING (auth.jwt() ->> 'role' IN ('admin', 'økonomi'));
CREATE POLICY ref_sessions_raw_delete ON ref_sessions_raw FOR DELETE
  USING (auth.jwt() ->> 'role' IN ('admin', 'økonomi'));

-- Sessions Hourly: ansatt ser kun egne, admin/økonomi ser allt i org
CREATE POLICY ref_sessions_hourly_select ON ref_sessions_hourly FOR SELECT
  USING (employee_id = auth.uid() OR auth.jwt() ->> 'role' IN ('admin', 'økonomi'));

CREATE POLICY ref_sessions_hourly_insert ON ref_sessions_hourly FOR INSERT
  WITH CHECK (auth.jwt() ->> 'role' IN ('admin', 'økonomi'));
CREATE POLICY ref_sessions_hourly_update ON ref_sessions_hourly FOR UPDATE
  USING (auth.jwt() ->> 'role' IN ('admin', 'økonomi'));
CREATE POLICY ref_sessions_hourly_delete ON ref_sessions_hourly FOR DELETE
  USING (auth.jwt() ->> 'role' IN ('admin', 'økonomi'));

-- Reimbursements: ansatt ser kun egne, admin/økonomi ser allt i org
CREATE POLICY ref_reimbursements_select ON ref_reimbursements FOR SELECT
  USING (employee_id = auth.uid() OR auth.jwt() ->> 'role' IN ('admin', 'økonomi'));

CREATE POLICY ref_reimbursements_insert ON ref_reimbursements FOR INSERT
  WITH CHECK (auth.jwt() ->> 'role' IN ('admin', 'økonomi'));
CREATE POLICY ref_reimbursements_update ON ref_reimbursements FOR UPDATE
  USING (auth.jwt() ->> 'role' IN ('admin', 'økonomi'));
CREATE POLICY ref_reimbursements_delete ON ref_reimbursements FOR DELETE
  USING (auth.jwt() ->> 'role' IN ('admin', 'økonomi'));

-- Effect Tiers: admin/økonomi ser allt i org, ansatt ser ikke
CREATE POLICY ref_effect_tiers_select ON ref_effect_tiers FOR SELECT
  USING (auth.jwt() ->> 'role' IN ('admin', 'økonomi'));

CREATE POLICY ref_effect_tiers_insert ON ref_effect_tiers FOR INSERT
  WITH CHECK (auth.jwt() ->> 'role' IN ('admin', 'økonomi'));
CREATE POLICY ref_effect_tiers_update ON ref_effect_tiers FOR UPDATE
  USING (auth.jwt() ->> 'role' IN ('admin', 'økonomi'));
CREATE POLICY ref_effect_tiers_delete ON ref_effect_tiers FOR DELETE
  USING (auth.jwt() ->> 'role' IN ('admin', 'økonomi'));

-- Profile Modules: ansatt ser kun egne, admin ser allt
CREATE POLICY profile_modules_select ON profile_modules FOR SELECT
  USING (profile_id = auth.uid() OR auth.jwt() ->> 'role' = 'admin');

CREATE POLICY profile_modules_insert ON profile_modules FOR INSERT
  WITH CHECK (auth.jwt() ->> 'role' = 'admin');
CREATE POLICY profile_modules_update ON profile_modules FOR UPDATE
  USING (auth.jwt() ->> 'role' = 'admin');
CREATE POLICY profile_modules_delete ON profile_modules FOR DELETE
  USING (auth.jwt() ->> 'role' = 'admin');
